...
def attention_han(inputs, attention_size, scope="attention_han",reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
...
def biGRU(inputs, inputs_len, state_size_list, return_mode, keep_prob=1.0,scope='biGRU', reuse=False):
    """
    :param return_mode: 0 - 分别返回前向、后向序列
    1 - 返回拼接后序列
    2 - 返回拼接后序列最后一个时间步的输出
    """
...
def conv_with_max_pool(X_embedded,filter_size_list, filter_num,
                       with_max_pooling,scope="conv_with_max_pool",reuse=False):
    with tf.variable_scope(scope,reuse=reuse):
...
def attention_to(Q, A, scope="attention_to", reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
...
def linear_transform(inputs,out_dim,activation=None,
